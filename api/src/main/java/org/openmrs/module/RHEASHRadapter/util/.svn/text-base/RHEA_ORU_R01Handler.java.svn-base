...
public class RHEA_ORU_R01Handler implements Application {
	...
	private Encounter createEncounter(MSH msh, Patient patient, PV1 pv1, ORC orc) throws HL7Exception {
		...
		if (encounterId != null) {
			...
			EncounterType encounterType = getEncounterType(pv1, form);
	...
	private Person getProvider(PV1 pv1) throws HL7Exception {
		...
		if (providerId == null){
			log.info("ID extracted from the HL7 message does not match with SHR records, a basic patient will be created...");
	...
	private Location getLocation(PV1 pv1) throws HL7Exception {
		...
		//If the Location does not exist, then create it and store it in the database
		if (locationId == null){
			Location location = new Location();
			location.setName(hl7Location.getPointOfCare().getValue());
			location.setDescription("Missing location specified via HL7 message : " + hl7Location.getFacility().getUniversalID().getValue());
	...
	private Form getForm(MSH msh) throws HL7Exception {
		...
		if(msh.getMessageProfileIdentifier(0).getEntityIdentifier().getValue() != null){
			try {
	...
	private EncounterType getEncounterType(PV1 pv1, Form form) {
		if (form != null){
